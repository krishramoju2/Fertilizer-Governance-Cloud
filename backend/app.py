...
def fetch_weather_for_location(location):
    ...
    try:
        ...
        if not weather_response.get('current'):
        ...
        logger.info(f"Weather fetched for {location}: {temperature}°C, {humidity}%")
...
def register():
    ...
    try:
        ...
        # Get location from registration data  <-- NEW
        location = data.get('location', '').strip()  # <-- NEW
        
        # Fetch weather data for the location  <-- NEW
        temperature, humidity = fetch_weather_for_location(location)  # <-- NEW
        ...
        user = {
            ...
            'farm_details': {
                ...
                'location': location,
                'primary_crops': data.get('primary_crops', []),
                # NEW: Weather data fields  <-- NEW
                'temperature': temperature if temperature is not None else 26,  # <-- NEW (fallback to 26)
                'humidity': humidity if humidity is not None else 45,           # <-- NEW (fallback to 45)
                'last_weather_update': datetime.datetime.utcnow().isoformat() if temperature else None  # <-- NEW
            },
            'is_admin': False,
            'created_at': datetime.datetime.utcnow()
        }
...
def update_farm(**kwargs):
    try:
        ...
        farm_details = {
            ...
            'location': data.get('location', current_user.get('farm_details', {}).get('location', '')),
            'primary_crops': data.get('primary_crops', current_user.get('farm_details', {}).get('primary_crops', [])),
            # Preserve weather data when updating farm details  <-- NEW
            'temperature': current_user.get('farm_details', {}).get('temperature', 26),  # <-- NEW
            'humidity': current_user.get('farm_details', {}).get('humidity', 45),        # <-- NEW
            'last_weather_update': current_user.get('farm_details', {}).get('last_weather_update')  # <-- NEW
        }
...
def predict(**kwargs):
    try:
        ...
        # Get farm details for defaults (including weather data)  <-- NEW
        farm_details = current_user.get('farm_details', {})

        # Prepare input data - using stored weather as defaults  <-- MODIFIED
        input_data = {
            'Temparature': float(data.get('Temparature', farm_details.get('temperature', 26))),  # <-- CHANGED: uses stored temp
            'Moisture': float(data.get('Moisture', farm_details.get('humidity', 45))),           # <-- CHANGED: uses stored humidity
            'Soil_Type': data.get('Soil_Type', farm_details.get('soil_type', 'Loamy')),
            'Crop_Type': data.get('Crop_Type', 'Maize'),
            'Fertilizer_Name': data.get('Fertilizer_Name', 'Urea'),
            'Fertilizer_Quantity': float(data.get('Fertilizer_Quantity', 30))
