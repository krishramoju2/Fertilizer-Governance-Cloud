...
Dataset made by three Indian students using various indian farm datasets
...
Expected Growth- EG = (TF × 0.4) + (MF × 0.35) + (FM × 0.25) Where: TF = Temperature Factor (1.0 if 25-32°C else 0.6/0.2) | MF = Moisture Factor (1.0 if 35-50% else 0.6/0.2) | FM = Fertilizer Match Score

Confidence PS = int(P × 100) Where: P = Random Forest predict_proba() output [0 to 1] 

Trust Index TI = (C × 0.4) + (TR × 0.2) + (MR × 0.2) + (QR × 0.1) + (SR × 0.1) Where: C = Confidence | TR = Temp Reliability | MR = Moisture Reliability | QR = Quantity Reliability | SR = Soil Reliability
